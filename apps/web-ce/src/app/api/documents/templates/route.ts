    ...
    try {
        ...
        if (memberError || !member?.organization_id) {
            return NextResponse.json(
                { success: false, error: 'No se encontró organización activa vinculada' },
                { status: 400 }
            )
        }
        ...
        if (error) {
            console.error('Error fetching document templates in GET /api/documents/templates:', error)
            return NextResponse.json({ success: false, error: error.message }, { status: 500 })
        }

        return NextResponse.json({
            success: true,
            data: (data || []) as unknown as DocumentTemplateModel[],
        })
    } catch (error) {
